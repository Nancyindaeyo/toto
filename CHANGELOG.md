# 1.5.68 · 手机翻页重说后不再一直 1/1

- 五版栈按「这层聊天 + 这楼 + 这张 swipe」存，不再跟着正文指纹换钥匙。手机端点箭头生成新版后应变成 2/2、3/3，而不是每次都 1/1。
- 旧的带指纹钥匙会在读取时并回这一楼；本机配额写失败时内存里仍留着当前会话的各版。
- 覆盖安装后请完整刷新，确认运行版本 1.5.68。

# 1.5.67 · 180 秒未完成就中止并自动重 roll

- 每一轮副 API 请求若 180 秒还没拿到完整兔子镜，中止本轮（流式碎片不算完成）。自动生成会按设置再发，默认最多 2 次额外请求。
- 空闲 5 分钟 / 总等待 20 分钟仍是终点：不再自动重发，需手动重说。
- 覆盖安装后请完整刷新，确认运行版本 1.5.67。

# 1.5.66 · 手机翻页条挪到标题开头 / 超时不再自动重 roll

- 翻页条改挂在标题 `▶` 旁边，不再塞进右侧星标/兔子那一组。手机长标题只会挤标题文字，不会再把 `‹ › ×` 挤没。
- 副 API 空回、报错、掉格式仍会自动重发（默认最多 2 次额外请求）。空闲 5 分钟或总等待 20 分钟超时后不再自动重发，和提示文案一致。没有 3 分钟档。
- 覆盖安装后请完整刷新，确认运行版本 1.5.66。
