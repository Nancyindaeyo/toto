# 1.5.65 · 标题箭头重说 / 手机翻页条 / 魔杖跟主题 / 收藏夹复制

- 标题 `‹ n/n ›` 到头或到尾时点箭头就是重说，不必再进挨打猫。满五版后下一版仍停住。
- 手机端工具条改成单独一行，左右箭头和 × 不再被长标题挤掉。
- 魔杖菜单「兔子镜」不再用按钮默认底色，跟着酒馆主题走。
- 收藏夹回看去掉标题上的翻页条、星标、兔/猫/骰子；多一个「复制 HTML」。旧收藏标题里残留的 `‹1/1›×` 也会清掉。
- 覆盖安装后请完整刷新，确认运行版本 1.5.65。
...
- 补上被截断的括号，缓存戳升到 1.5.63。覆盖安装后请完整刷新，确认运行版本 1.5.63。

# 1.5.62 · 拆分重复导出导致核心加载失败

- 万行拆分把 `scanCurrentChatIndependentContextTags` 导出了两次，浏览器抛 SyntaxError，延迟核心整包加载失败。表现：魔杖「打开失败」、旧小剧场不显示、无法生成。
- 去掉多余的 `export { ... }`，延迟核心相关缓存戳升到 1.5.62。覆盖安装后请完整刷新，确认运行版本 1.5.62。

# 1.5.61 · 万行源码拆分（行为不变）

- 把净化器、副 API、设置页从整份万行文件拆成目录；旧路径仍是入口/桶文件，延迟加载图不用改。
- 不改生成、重说五版、失败卡不占格、维修恢复到初始、收藏、Prompt、自动重 roll。
- 覆盖安装后请完整刷新，确认运行版本 1.5.61。请点一次：生成、重说五版、失败卡、维修、标题星标、设置开关。

# 1.5.60 · 重说五版切换 / 标题收藏 / 可调自动重 roll

- 副 API 成功重说按「每一面」最多保留 5 版，标题旁 `‹ 2/5 ›` 切换；满五后必须先删当前这一版才能再重说，不会自动挤掉最旧。
- 维修不再另存版本。维修兔「恢复到初始」回到这一版刚生成时的整面 HTML，格子数不变；「返回修复前」和挨打猫「兔子镜历史」藏掉。
- 重说失败只显示失败卡，不写入那 5 格；切回上一版或刷新后回到最近成功成品。失败卡上再点重说覆盖这一格失败。
- 标题兔子按钮旁增加收藏星标，点第二次取消收藏。扩展魔杖菜单「兔子镜」补上同款兔子图标。
- 设置里可调副 API 自动重 roll 次数，默认仍是 2。

# 1.5.59 · 收藏夹回看不再闪退

- 收藏夹 / 回看面板改为顶层对话框，能盖在设置窗口上面；Esc 只关收藏夹，不会把整个设置一起关掉。
- 回看成品不再当成聊天里的独立镜面：不会被切模式清掉，也不会被外置几何拖出卡片。点小剧场 HTML、表单按钮不会把面板秒关。
- 设置皮肤与收藏夹模块更新缓存戳，覆盖安装后能看到收藏夹分页。新手指引定位「兔子镜收藏夹」会打开对应页面。
- 独立 API 自动重 roll 用尽后才提示「已自动重试 2/2」；第一次失败或因已有鲜面而跳过时不再误报。

# 1.5.58 · 维修兔重排 / 收藏夹按角色卡回看

- 维修兔菜单按游玩高频问题重排：点了没反应、展开后文字被裁、整面太窄。新的「展开后文字被裁」只测量当前已打开的抽屉／选项，放开纵向 max-height 与裁切祖先，不预拉开折叠内容。
- 「点了没有反应」修完后用人话回执：已接上开关、没接到第二层、或其实已经打开但被裁（改走展开后解裁）。
- 兔子镜收藏夹从抽签「收藏室」里拆出，设置首页单独一栏，并在镜面工具菜单增加「打开收藏夹」。列表按角色卡分组；旧收藏没有角色信息时归入「未分类」。
- 不增加自动巡检、不改生成 Prompt、不接七日历。

# 1.5.57 · fork 重 roll / 收藏夹 / 可见楼层回挂

- 安装通道改为 `https://github.com/Nancyindaeyo/toto`。内置「检查并更新」仍走宿主已登记的 git remote，不读 homePage。
- 独立 API：本轮没有可用鲜兔镜时自动重发最多 2 次（合计最多 3 次请求），之后需手动重试；已出的 ready 镜面不会被失败卡或 loading 壳盖掉。
- 本机收藏夹保存已净化的单面 HTML/文本，打开时走原净化和挂载管线，保留交互；不写入主楼 Prompt。
- TT 虚化：滚出屏幕只摘 DOM、保留缓存成品；滚入可见楼层时按 mesid 回挂，不发新请求。这不是 ChatSurface 晚注册已修好。
- 工具菜单 z-index 降到输入栏之下，仍按 visualViewport 定位；独立标签名改用 `toLowerCase()`。

# 1.5.56 · fork 合入作者 1.5.53

- 在保留 TT DOM 虚化轻壳外置（原 fork 1.5.52–1.5.55）的前提下，合入作者 1.5.53：镜面生图、文字面、设置皮肤工作台、工具菜单、副 API 定时、中文标签边界。
- 作者原包未单独写 1.5.52/1.5.53 changelog；本 fork 版本号升到 1.5.56，避免与上游 1.5.53 和旧 fork 1.5.55 混淆。
- 生图依赖柏宝绘公开 API v1；工具菜单与设置改为 body 级弹层/对话框；TT 可见楼层回退路径忽略生图节点，避免把插图 DOM 当成宿主正文提交。
- 仅源码合并与既有离线测试；云酒馆 / 本地酒馆 / TT 真机未在本轮验收。

# 1.5.51 · NarrowFaceShock1 窄面电击候选

- 维修兔新增「⚡ 强效电击：恢复窄面」，仅显式点击执行，不增加启动巡检。
- 补上精确 owner 外置宿主的手动几何复测；正文宽度恢复覆盖跟随／独立、外置／内嵌路径，沿用 TT 的宿主定位接口。
- 保留作者明确的卡片尺寸、浮动与定位意图；正文尺寸写入若未实际增宽则撤回，不把无变化报成已修复。
- 直接保留当前 DOM、已选控件与交互事件；不进入重新生成、源码重建或成品保存流程。
- 继承 1.5.50 多面登记体积修复；生成 Prompt、抽签、请求授权、传输解析与 TT 加载时序逻辑未改。
- 几何自动化使用模拟宿主/尺寸，不是浏览器布局引擎或云/本地/TT实机测试；仍需验证真实显示。

# 1.5.50 · QuotaFix1 多面登记空间修复候选

- 新批次采用有界、无损的紧凑编码保存完整冻结计划；读取时原样恢复。只有 UTF-16 字符数和 UTF-8 字节数均减少时采用压缩，否则保留原文。
- 新登记使用 v3 独立槽，兼容读取、提交与释放 v2 的旧任务；不会把压缩记录交给旧运行时误读。
- 新成功历史仅去掉主题／形式的完整材料副本，保留 ID、UI 重点、交互、视觉指纹、冷却与批次状态。
- 成功提交先完成缩小的写入，再增加历史；失败保留原所有权回滚，减少不必要的瞬时容量需求。
- 未修改生成 Prompt、抽签算法、请求参数、响应解析、DOM、CSS、维修兔或挨打猫。未增加模型请求、轮询或自动清理成品。
- 40 项定向自动化测试通过；原包未附带历史全量测试。仅完成源码／自动化验证，尚未真机验证。

# 1.5.48 · 正式通道分离

- 正式包使用正常 index 入口；移除 TT 备用设置栏、备用“打开设置与诊断”及独立小剧场查看器，不是仅隐藏按钮。正常完整设置与原有诊断仍保留。
- 保留本版外部母本修复、TXT 导入、高级独立 API 参数、生成、维修、挨打猫及既有数据；没有新增请求、后台扫描或存储迁移。
- 正式包目标为 main，TT 测试版独立保留；没有上传或切换远端分支。TT late-projection 未在本次解决，尚未真机验证。

# 1.5.48 · 外部母本修复 / TT 测试（历史记录）

- 修复部分点菜的随机半边缓存未随启用库/可用条目变化失效的问题；新的生成按当前可用库选择，同一进行中的请求仍冻结，不重抽、不重发。
- 新导入的自动摘要优先正文，避免长标题/关键词挤掉内容。符合旧自动摘要缺陷的已选条目只修正发送副本，不迁移库、不改存档原文。
- 未增加诊断功能、抽取概率、Prompt 预算或模型请求。保留 1.5.47 TXT 导入与 TT 独立查看功能；TT 虚化内嵌和真实模型遵从未验收。

# 1.5.47 · TT 测试

- 文件导入提供明确的按钮，不再依赖被宿主隐藏的原生文件输入框旁边的文字标签；可重复选择同一文件。读取、保存、启用仍分步确认。
- 修复明确带 BOM 的 UTF-16 LE/BE TXT、MD、JSON 解码；保留 UTF-8、单文件单条目和大小限制，不猜 GBK 等编码。
- 仅 TT 新增隔离的已有小剧场查看面板，按页读取当前聊天，不生成、不迁移、不改聊天记录；脚本/外部媒体不在此面板执行。
- 全链路诊断从只显示首面改为逐面列内置/外部选材数量；选材记录不等于最终发送或模型遵从。
- TT late-projection 原位置挂载仍未修复，未关闭虚化或重试登记。未发送真实模型请求，尚未真机验证。

# 1.5.42

- 修复 TT 非 managed 模式下，触摸快速展开后，同 pointerId 的延迟 click 被宿主标为 mouse 而再次收起的问题；真实鼠标操作、键盘、云酒馆和 managed TT 原路径保留。
- TT managed 模式提供不依赖聊天挂载的轻量设置入口，明确点击才加载完整设置；设置入口恢复不等于晚注册问题已经解决。
- 没有关闭虚化、修改 TT 宿主、扫描完整聊天或新增付费请求。记忆世界书仍单本绑定；HTML 导出仍不包含插件脚本补丁。
- 翻译替换后的失败卡等待状态完成独立诊断，本包没有修改生成链。仅源码/自动化验证，尚未 TT 真机验证。

# 1.5.41

- 已导入母本库可逐条勾选参与抽签，持久保存、不删除原文；单库分页与手动标题搜索，不增加启动整库读取。
- 维修兔增加「复制本面 HTML（含样式）」；浏览器剪贴板不可用时可手动复制，不附带整份诊断。
- 高级共同回忆资料来源增加记忆世界书绑定；跟随、独立 API 和显式单面重试仅抽中 I.1 时读取，按实际材料计入 Prompt，读取时改设置或归属则停止使用过期结果。
- TT 注册失败细分为晚注册、重复注册或其他宿主拒绝。已定位上游启动竞态，但插件 ZIP 不能单独根治；保留 managed ownership，不关闭虚化、不强扫历史。
- 正式/测试仅通道元数据不同；无付费模型调用、远程部署或 TT 真机验收。

# 1.5.38

- 补充创作规则移到高级设置的独立 API 首部；外围说明仅留作用域，原文和用户保存内容不变。
- 设置首页新增母本库导入与备份分区，粘贴文字、文件、整库换设备和管理各自直达；新手指引同步。
- 删除独立生成、跟随生成、历史恢复与重说中的审美拒收调用；保留安全净化、完整性、归属与预算检查。缺面、空正文等不再误报“质量检查未通过”。
- 旧 ZIP 和数据保留，不推送或部署；真实手机/TT 与模型效果不能用离线验证冒充。

# 1.5.37

- 整库迁移备份可直接从普通本地导入入口识别，经确认原子保存，已有同编号库保留。
- 补充创作规则移至独立 API 显著位置；感谢 @lannuomi，默认完整保留原文与注释，已编辑/清空内容不覆盖，仅副 API 单份注入。
- 提前生成不同 Profile 缺少独立凭据绑定时在发送前提示；错误分类不再泛指 Secret，失败后开启外部诊断可读最近标量回执。
- 复核 TXT/MD/JSON/粘贴到分类、保存、启用、抽中材料读取；不代表真实服务商或手机验收。详见 docs/update-1.5.37.md。

# 1.5.36

- 修复多面被动同步反复搬动节点，导致长文拖动回顶与弹幕动画重启。已正确归位的面不再移动。
- 桌面兔子镜单面最大阅读宽度 720px；不修改宿主正文、手机样式和作品字体。
- 当前创作补充规则可编辑、清空、关闭和恢复默认，支持每轮／仅抽到成人内容注入；取消重复的隐藏成人执行锁，保留母本具体内容。
- 外部库支持整包导出／导入，以及 TXT、Markdown、粘贴文本。分类流程、IndexedDB 懒加载保持；迁移同编号库跳过，原有库不覆盖。
- 独立 API 新增默认关闭、绑定当前聊天的正文标签闭合早发请求选项；保持一轮最多一次请求，结果等主正文结束且归属未变后显示。
- 已有冷却开启时，同批多面增加主交互分散偏好，不以交互重复拒收作品。
- 测试版名称固定为“兔子镜小剧场测试版”，正式版名称固定为“兔子镜小剧场”。使用及验收边界见 docs/update-1.5.36.md。

# 1.5.35 稳定性候选

- 现有外部诊断可记录同一次独立 API 响应的状态码、Content-Type、实际解析方式、已读字节、content 字符数、结束类型及标准化 finish_reason。不记录正文、Key，不追加请求。宿主未提供的信息显示未知，不伪造成功状态。
- 普通酒馆避免对同一未更换节点重复安装交互；TT 托管消息通过公开生命周期处理挂载与卸载，外置镜面和留空属于各自消息，不在虚化列表根部插入额外节点。修正相关计时器、事件及引用清理。
- 配额修复仅调整既有事务中“缩小项先写”的顺序，降低瞬时峰值；历史数据、有效事务和其他插件仍保留。真正空间不足仍停止，不重发 API。
- 增加默认关闭的外观与交互参考，仅在用户保存时提取有界的结构、样式和控件关系摘要；摘要留在本设备 IndexedDB，settings 只保存开关与版本引用。正文继续由当前聊天生成。
- TT 设置文字高度与移动弹窗标记做局部适配；不修改主题素材，不关闭 DOM 虚化，不恢复审美质量拦截。
- 未包含破限规则编辑、母本清理、跨设备母本存储和正文标签提前生成。真实设备滚动、闪烁、温升、TT 版本组合及模型输出速度仍待验收，不承诺固定提速。

# 1.5.34 外部抽签与塔罗误触发

- 保留 1.5.33 的成品展示策略，不恢复审美拦截、不另造美化规则。
- 塔罗专用规则只由明确的塔罗媒介或题材启用；否定、可选举例、随口提及和来源关键词本身不改变普通外部媒介。
- 外部冷却全部耗尽后，根据既有精确条目命中次数降低高频项机会。主题与展现、正常抽取与补足路径均适用；不固定轮播、不永久排除候选。
- 无历史、频次相同、关闭避重复时沿用原分布；关闭外部时内置随机不变。保留同批硬排除和单次请求，旧库无需重导，无新存储、全库扫描或 API 请求。
- 同类语义去重未实现；条目本身相似、候选过少和模型偏好仍可能重复。真实手机表现与模型审美需要实机确认。

# 1.5.33 保留成品与旧预览展开

- 基于 1.5.32，取消三标签/普通版式、单一揭示、低对比、缺牌图、视觉 CSS 缺失及去标题后的正文重复造成的质量失败；安全可用的现有内容直接保留。
- 审美观察不触发失败卡、重发请求或强制重新生成。保留现有生成前的媒介交互、反三标签模板和自然返回规则；不扩充 Prompt，不保证模型每次遵守。
- 旧失败卡预览缓存仍有效时，点击后直接打开作品外层；不强行展开内部交互，不改变邻面。缓存失效时明确禁用预览，不能恢复旧版未保留的原文。
- 不放宽脚本/外链净化、协议闭合、空正文、体积、逐面身份和单次付费请求保护；不改存储、主题/母本、手机布局、模型参数或动画预算。
- 正式/测试 ZIP 代码一致，通道独立；交付不等于上传或部署。真实手机、模型生成效果与速度未验收。

# 1.5.32 交互返回规则合并

- 基于 1.5.31，仅合并交互返回约束，保留完整交互链和既有视觉要求。
- 可重复交互须能自然切回，优先复用原控件；不要求每个中间状态另造返回入口，一次性动作可自然结束。
- 跟随/独立、普通/动态场景、增强绘制及多面短锁采用同一口径，不再以重复“可逆”要求强制所有动作返回。
- 不改生成参数、API 次数、抽签、母本、净化、存储或布局；不新增轮询、观察器和后台工作。旧成品不会因本次规则调整自动改写。
- 这是减少不必要生成约束的修订，不承诺具体提速、生成效果或手机温升。交付 ZIP 不代表已经上传或部署。

# 1.5.31 过期事务回收与美化兼容微调

- 基于 1.5.30；保留标题浮动、局部交互、宽度边界与指引修复。美化提示、内置母本、抽取规则及模型请求参数不变。
- 多面请求登记写入遇 QuotaExceededError，仅清理白名单内、结构可确认且已超过 12 小时的兔子镜临时数据，重试同一计划的一次本地写入。仍失败则阻断请求，不重抽、不切 nostream、不追加 API 请求。
- 不清理历史成品/维修结果、不清站点、不碰其他扩展；有效异页事务、未知结构与异常时间戳不作为清理依据。localStorage 不具备原子跨标签锁，遇可检测的外部写入时停止恢复。
- 完整孤立的裸露 @keyframes 恢复到原有安全样式管线；保留正文和代码示例，逐面隔离动画名。
- 最后一条正文有伸出框外的页脚或翻页区域时，按实际伸出距离补回少量间距；只读几何，不包含第三方主题或素材。
- 交付不代表在线部署；手机实际温升、全部主题兼容和双向跨设备保存未完成验收。详见 docs/repair-1.5.31.md。

# 1.5.30 标题浮动与局部交互修复

- 基于保留的 1.5.29；功能改动仅在 outputSanitizer.js，其余运行模块只更新版本及缓存标记。
- 标题内工具浮动增加自身边界收尾，不让后续正文因避让工具而变窄。保留作者边框、配色、阴影及伪元素装饰，不泛化拉宽有意设计的小卡片。
- 对已证明有界、裁切且具备进入/返回入口的局部画布恢复第二层定位，避免交互状态已切换但内容仍在可见区外。任意脚本与固定/视口覆盖层的安全限制不放宽。
- 新增原创触摸布局与交互回归；安装包不包含测试、用户诊断、截图、主题或私人母本。未向模型发送请求、未上传仓库。
- 跨设备保存仍未补齐；真实手机的联动、温升、启动/生成耗时仍待验收。

# 1.5.29 维修保存对应关系、外置间隔与指引定位

- 基于保留的 1.5.28。独立模式保存维修结果时，用唯一节点身份/结构匹配初始状态，不再沿整个 DOM 的标签顺序猜测对应节点；控件、选项和内层折叠项也按身份恢复。
- 排除完整的运行时工具子树，保留维修新增的有效结构，保持当前 CSS 作用域；不重写美化提示或抽签规则。
- 仅对紧邻可见独立外置镜面的最后一条正文移除多余底部 margin，保留输入框防遮挡机制。
- 指引导航等待已有设置懒加载完成，定位并突出实际控件，不滚到返回栏。
- 原创结构的离线回归覆盖插入容器、控件换序、歧义拒绝、作用域变化、五面维修保存、重新挂载和局部返回。未调用真实 API，未上传仓库。
- 本版不是手机原故障、温升、启动/生成速度或跨设备恢复的完整验收。已损坏且无完好初始副本的旧成品不能保证恢复。

# 1.5.28 分模式新手指引与功能导航

- 基于保留的 1.5.27；“新手指引”默认一行折叠，首次打开才加载内容。
- 跟随 / 独立分别说明，均先开启自动注入；独立流程包含连接、模型、读取层数、过滤标签与显示方式。
- 完整功能目录分六类、39 项；点击只定位已有设置，返回保留分类展开状态和阅读位置。
- 查看指引不保存设置、不扫描聊天或母本、不发 API 请求；镜面工具说明具体入口，不擅自重生成或维修。
- 动态视觉增加审美疲劳提醒；美化提示、抽签、存储、请求、净化及维修实现不变。
- 正式与测试安装包运行代码一致，不含 tests/scripts/package.json；仅 manifest 通道字段不同。手机实机与跨设备未完成项仍保留。

# 1.5.27 底栏适配、重复恢复修复与小型帮助入口

- 按聊天可见区域与输入框的实际重叠量，提供透明、可滚动的底部留空，不统一套框或修改原作品配色、美化 Prompt。
- 修复内部工具/交互更新时，已移除的子节点被误判为宿主删除消息的问题；真正的消息重建仍走原恢复流程。安卓视频的全部闪烁原因尚未实机确认。
- 设置顶部新增“检查并更新”，只更新当前安装的扩展，合并重复点击；明确提示宿主权限、安装或网络错误，不删除重装，不自动刷新。
- 新增默认一行折叠的新手指引，展开后查看四步说明，并复用原一键正则配置入口。
- 无真实模型请求；不改变母本保存位置、生成参数、多面解析或维修生成规则。手机发热、Safari/安卓本机效果仍需实际验收。
- 详细范围与离线验证方式见 docs/mobilefix-verification.md。

# 1.5.26 来源文案、输出契约与移动导入窗口

- 来源注释改为“本轮抽取来源：书名”，半透明显示，关闭外部抽签时立即隐藏，不改作品标题。
- 外部母本设置页和导入页最前面增加用户指定的作者许可提醒。
- 独立 API 共享 Prompt 的正文预留及输出顺序改为观察已完成正文，不再要求先续写正文；跟随规则保持原样，美化规则不变。`outside-content` 提示解释面外文字/注释，严格解析和单次请求不变。
- 导入弹窗使用可见视口尺寸与偏移，修复可见区变化时关闭入口越界；关闭即移除事件监听，不常驻轮询或扫描聊天。
- 具体离线证据与实机限制见 docs/compatfix-verification.md。未调用真实模型、未更新在线酒馆或仓库。

# 1.5.25 来源标注与交互优化

- 撤回统一的逐面外框和 6px 内边距；外置层不再强制清掉作品本身的边框、背景及圆角。
- 标题使用包含浮动的布局，允许自然换行增高，避免维修等图标落在背景外。
- details 内容层使用可用宽度；`width:auto` / `inline-size:auto` 不再被误判为固定窄面。明确的固定宽度及 fit-content 保留。
- 外部参考在镜面底部注明实际抽中的世界书，逐面绑定并保留原名；不向 Prompt 添加书名或额外说明，不扫描未抽中的母本。
- 正常点击后的延迟校验不再反复恢复、重写已经正确的样式；仍保留状态回退补救及快速切换保护。
- 全量 Node、布局及交互验证见 docs/layoutfix-verification.md。仅离线原创夹具；实际手机发热和长延迟仍待确认。
- 未改生成美化提示、母本正文、请求参数或所有权校验。安装包交付不表示已上传仓库。

# 1.5.23 UI / Mobile Fix

- 最近请求抽签记录去除内部 ID，仅显示短名称并逐面列出；旧版记录兼容，缺失名称不伪造。原始抽签材料、ID 与请求保持不变。
- 修订镜面边界及窄屏宽度保护，不新增页面扫描、动画、网络请求或生成提示。
- 外部世界书母本及禁词替换移到高级设置底部；导入入口补充“小剧场世界书”用途。
- 生成调度、网络、Prompt 和安全边界未改。电脑离线浏览器测试不能证明 iPhone 温升与长延迟已全部解决；跨设备成品及维修同步仍未完成。

# 1.5.22 公开发布包（2026-09-06）

- 用户试用后要求整理供正式仓库上传。名称为“兔子镜小剧场”，主页改为正式仓库 toto，按正式通道约定开启更新；版本及缓存保持 1.5.22。
- 运行代码、Prompt、母本与安全边界和 BatchFix1 完全相同；仅调整发布标识和说明。安装包省略 tests、scripts、package.json，开发副本保留。
- 成品及维修结果跨设备同步尚未实现；手机温升、长延迟及真实模型视觉效果仍待验收。公开不等于所有问题已经解决。
- 本次只交付上传包，未推送正式仓库或更新在线酒馆。以下为历史记录。

# 1.5.22 BatchFix1（基于 1.5.21；手机实际验收待完成）

- 修复外部库连续多面抽取时将历史尝试当成硬排除的问题。历史只作优先避让，同批已用条目和用户明确排除仍为硬限制；不清空已有记录、不自动重抽或补请求。
- 新增原创 45 条外部库连续八轮五面回归，覆盖实际规划、提示构建、登记、消费及独立 API 发送前路径；真实不足、严格排除和容量保护继续拒绝。
- 保留 1.5.21 首次交互与本地维修结果修复，仅统一新版本和缓存标识。未更改 Prompt、美化、性能或跨设备同步实现。
- 旧手机提示可能还含其他分支，本包不将实验室通过等同于手机故障全部解决；跨设备双向恢复和手机温升继续待验收。

# 1.5.21 InteractionFix1（测试候选；跨设备未完成）

- 新成品的 ID 隔离不再被误当作已经完成交互绑定。当前面首次展开后或首次内部操作前安装既有安全交互，后续展开不重复扫描；不运行自动维修诊断或自动保存，不执行模型脚本。
- 修复 `currentGenerationIdentity` 缺少 index 导致单面/多面维修元数据回读必然失败。保存预算不足时保留原本地结果，不让压缩器先丢掉旧成品。
- 移除被动动效激活时额外写入的 `running!important`，保留作者状态和折叠暂停。
- 多面登记错误显示固定原因码，区分身份、池、登记、容量等问题；拒绝后不再追加误导的“生成中”通知。不放宽归属/单次请求边界，不自动重试。
- 这是局部修复测试包。成品及维修结果跨设备云端写入尚未补齐，手机→电脑与电脑→手机实际恢复仍未通过；手机温升、长延迟、真实模型美学仍待验收。

# 1.5.20 RuntimeFix1（测试候选）

- 修复中文长外部 ID 被批次验证误拒绝；固定阶段错误代码可见，母本正文不进入诊断。
- 外部预取期间宿主最终回写后，错误结算到当前同操作精确 hash，避免变成永久刷新提示；跨聊天、Swipe、操作或显式替换仍隔离。
- 导入弹窗改为原生顶层对话框，移动端安全区、滚动、关闭和焦点恢复独立处理；文字替换入口补 🚫。
- 普通展开不再进行诊断规模的裁字测量；同源交互不重复解码整条多面正文。扩大标题触控与面间距，不改成品配色。
- 动态场景与非场景交互指令分域，去掉画外文字替代画面变化的冲突规则；没有增加固定布局、付费请求或 token 上限。
- 不完整 raw-text 按实际结构/finish reason 解释，不自动补标签或重发。
- 保留成功面、单次请求、原抽签与安全净化；真实模型审美、五面手机卡顿与温升仍需验收。

# 1.5.19 Usability1（测试候选）

- 外部母本与文字替换拥有独立高级设置入口；文件导入、强线索分类、批量分类、旧库索引重建和窄屏导入窗口得到修订。
- 用户明确授权部分成功保存。保留合格面，失败面不写入成功／冷却历史，不自动补发；全局超限和不可信结构仍拒绝。
- 跟随模式失败面手动重试接入当前 Chat Completion 主 API，不改正文，不切连接；单面结果仍需归属、请求边界、净化和质量检查。
- 多面外置／内嵌往返保留控件状态；交互快照不再每次重复遍历已有或超预算镜面。
- 本地字面替换支持空值删除，修正解析／赋值的双重替换和含 `=>` 旧词规则回存。
- 动态提示解除固定单热点显隐链；人物绘制不再允许无法辨认的散落几何。整批输出上限表意更明确。
- 真实宿主、服务商、手机温升、复杂裁字与模型美学仍待实测。未覆盖旧安装包，未发布远程仓库。
...
- 修复连续两条回复时前一条完成 intent 被清除，以及同消息精确完成证据未被识别的问题；已生成镜面不再回退为“等待正文完成”。
- 独立 API 的 Profile、上下文、批次和大小预检在付费边界前失败时明确 `requestCount=0`，不再误报网络断流或建议 nostream；真正跨过单次 lease 后仍绝不自动重发。
- 依据真实 Edge／SillyTavern 复现修复多面执行锁与独立 API 安全检查边界名漂移：单面及 2～5 面统一使用同一完整边界；错序、重复、空正文或跨消息拼接仍在请求发送和 lease 消费前失败关闭。
- Connection Manager 字符串按增量计数；隐藏非字符串状态使用每帧 512 项／64 层／63 字段的固定工作量验证，验证后丢弃。响应过深、过大或过复杂以稳定的 `response-boundary` 结束，不做 O(n²) 全量重复序列化。
- 跟随当前 API 的外置多面等待全部净化 proof 与整批 commit，再凭精确消息身份同步；移动端宿主残留的 busy 标志不再挡住外置。
- 增强视觉明确 360px 正常流、内容撑高、换行和末行完整；当前最新宿主的每个 face 最多一次裁切检查（单批最多 5 次），只修独立简单安全的裁切祖先，复杂结构保留给维修兔诊断。
- 多面冻结亮度、色系、材质、外轮廓、阅读路径和交互家族；动态视觉保留一主一辅可见运动，同时暂停折叠面及伪元素动画、响应系统减少动态效果，降低移动端持续负载。
- 删除插件注入的通用“返回初始页”按钮；每个非终局第二状态改由媒介内部自然对象／动作返回，维修兔恢复与鼠标、触摸、键盘初态快照继续保留。
- Token 卡区分跟随／独立、最近／历史，并明确为 Prompt 本地估算而非账单；跟随显示待注入 Prompt，独立显示 system＋user 请求消息内容字符合计。
- runtime／manifest／CSS 为 `1.5.8`，核心模块 cache cohort 为 `1.5.8-visualstream8-boundary1`。跟随模式不新增独立模型请求；独立模式每轮仍只新增一次兔子镜请求，1～5 面共享且不自动重发。
- 仅完成源码与自动化验证；真实 SillyTavern、真实模型／网络、iPhone／Android 的视觉、温度和家庭网络影响仍需目标设备实测。GitHub 未写入，1.5.7 原包未覆盖。

# 1.5.7 Multiface5（测试候选）

- 开放 1～5 面自选，跟随当前 API 与独立 API 均为一轮一次请求；每面独立抽取、标题、UI、净化、历史身份、维修兔与挨打猫，禁止在同一个 `<toto>`／`<details>` 内混装多面。
- 后续面避开本批前面已经随机抽中的精确主题与展现形式；强制项／明确点菜仍按既有规则执行。多面计划按 owner 与设置指纹隔离，成功后聚合提交，失败不自动补面或重试。
- 独立模式一次响应支持 1～5 个平级成品；局部重说只替换目标面，邻面 DOM 与交互状态不重建。成功后的缓存／DOM 后置复核未命中只记录警告，不再把已生成成品改成失败或令其消失。
- 增强视觉绘制移到动态视觉场景旁并加强为一条共享短规则；可叠加，关闭时 Prompt 逐字保持原样，开启时不会按面重复注入。
- 塔罗／牌阵／西方神秘学要求官方 78 张实体牌图与中文替代文字；独立成品净化后缺少合规牌图时不记为成功。
- 恢复兔子镜内部“返回初始页”，并修复多面历史恢复、后续面 Safari 宽度救援、键盘控件初态快照及维修兔局部恢复。
- HTTP 200 provider error envelope 优先于其中夹带的正文；仍只发送一次请求且不会自动重发。
- runtime／manifest／CSS 为 `1.5.7`，核心模块 cache cohort 为 `1.5.7-multiface5`。
- 仅完成源码与自动化验证，尚未完成真实 SillyTavern、真实网络／模型与手机实机验证；GitHub 未写入。

# 1.5.6 ABC1（测试候选）

- A：普通格式 group 乘数改为 1；仅最终明确 IF 主题保留 group 冷却。格式 family/group 使用正式历史，exact 仍保护 attempt；family 系数、收藏/黑名单、母本、主题冷却和现有 pity 重置语义保留。
- B：新增严格布尔且默认 OFF 的增强视觉绘制开关。ON 只在原全局视觉地板后增加 65 字符媒介许可；OFF 不变，近输出锁不变，净化规则不变。
- C1：只补多面随机计划与存储身份/幂等/失败降级。面数 1 走旧路径，2/3 面基础 API 不接入生产请求或 DOM。C2 仅交付分期设计。
- 发布 runtime/manifest/CSS 身份为 1.5.6，现有核心模块 cohort 更新为 1.5.6-abc1，并验证真正反向父模块闭包。历史存储 key、绑定协议版本和未变叶子模块的旧缓存键保留。
- 新增定向回归、独立原包 Prompt fixtures、保护文件哈希对照；原母本一致性失败保留，不删除或放宽安全断言。存储无效面数与旧多面恢复测试按本轮严格契约加强，详见验证报告。
- 新增设置 change 监听 1 处；不增加常驻任务、Observer、计时器、网络请求或全聊天扫描。更新后整页刷新；未证明未清理旧入口的同页热更新完全安全。
- 仅完成源码、自动化测试与随机分布模拟验证，尚未完成真实 SillyTavern、真实模型网络及手机实机验证。未晋升正式版，未写 GitHub。
