...
- 副 API 自动重 roll：空回、报错、掉格式、净化失败或缺面都会再试；多面一次只补缺的那几面。连续没有新进度默认 90 秒中止再试，设置里重 roll 次数旁边可改。401 / 429 会重试，额度不足、发送前拦截、点停止、切聊天、正文被换掉不会。正文 swipe 会跟着重新生成，旧镜保留成 2/2。次数不再封顶。
