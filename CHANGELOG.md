# 1.6 · 合入作者窄修：镜内叠层不再被清掉，导入面板跟主题

- 副 API / 跟随正文 API 自动重 roll：设置里有总开关，打开后才出现次数和无进度秒数。空回、报错、掉格式、净化失败或缺面都会再试；多面一次只补缺的那几面。连续没有新进度默认 90 秒中止再试。401 / 429 会重试，额度不足、发送前拦截、点停止、切聊天、正文被换掉不会。正文 swipe 会跟着重新生成，旧镜保留成 2/2。次数不再封顶。关闭后，除了手动重新生成正文或手动重说，都不会自动再生成兔子镜。跟随正文 API 的补发走后台补请求，不会中止正在写的正文。
- 合入作者 1.5.53 窄修：镜面模板自己写了定位上下文时，内部 `absolute + 100%×100%` 叠放层保留 `position`；`fixed` / `sticky` 和高 z 整贴覆盖仍删除。维修兔不再把这类叠放外壳改成滚动盒或放开高度。
- 母本库「导入与备份」弹层套用设置面板同一套主题令牌；来源与备份分区，按钮统一 44px。
- 安装通道与 `homePage` 改回官方仓库 `https://github.com/Zaiyebuzuoyouqingdetiangou/toto`。
- 覆盖安装后请完整刷新，确认运行版本 1.6。

# 1.5.76 · 修好 1.5.75 把设置和小剧场一起关掉的问题

- 1.5.75 入口仍登记成 1.5.74，设置和挂载认为运行时不是当前版本，直接退出：魔术棒打不开，聊天里的小剧场也不再挂上。
- 多面翻页只在已经标了当前面时才隐藏其余面，避免还没标当前面时把所有卡都藏掉。
- 覆盖安装后请完整刷新，确认运行版本 1.5.76。

# 1.5.75 · 多面翻页不再叠出第二张卡

- 多面 `‹ 2/2 ›` 只应留下当前这一面。原先外嵌/内嵌壳把每面都强制 `display:block`，翻页后下面还会多出一张折叠卡。现按当前面隐藏其余面。
- 覆盖安装后请完整刷新，确认运行版本 1.5.75。

# 1.5.74 · TT 外嵌回到正文栏下方

- TT 不能把外壳插成 `#chat` 的兄弟节点。1.5.73 挂到横向 flex 的 `.mes` 里，成品和失败卡都被挤到头像行右侧，只剩一个小三角。现改回挂在 `.mes_block` 里、`.mes_text` 下面，和原先外嵌一样铺满正文栏。
- 覆盖安装后请完整刷新，确认运行版本 1.5.74。

# 1.5.73 · 缺壳设置进工具页，TT 失败卡挂在整层消息上

- 「缺壳扫描范围」「重新检查缺壳楼层」挪到工具 → 镜面出问题了；原先藏在副 API 显示模式那张会被分层 UI 隐藏的卡片里。
- TT 不再把失败卡塞进会被正文/日历改写的 `.mes_block`。外壳作为 `.mes` 的最后一个子节点，换行铺满整层；正在看的楼层即使不在扫描范围内也会补卡。
- 完整请求字符预算在设置 → 生成参数，和温度、整批最大输出、自动重 roll 放在一起。
- 多面不再上下叠卡：3 面就是标题旁 `‹ 3/3 ›`。从单面改成多面后，刷新不再丢掉后面几面。
- 多面请求遇本地存储配额不足时，会先压缩本扩展的翻页缓存和旧成品再登记；仍满则阻断请求，不发网络、不切 nostream、不清除站点数据。
- 覆盖安装后请完整刷新，确认运行版本 1.5.73。

# 1.5.72 · 请求字符预算可调 / 多面合并翻页 / 配额自救

- 完整请求字符预算默认从 32000 提到 50000，设置 → 生成参数里可改，不再写死。旧安装未保存过该字段时自动用新默认。超限仍只做本地预检，不发请求。
- 多面不再上下叠多张卡。3 面就是标题旁 `‹ 3/3 ›` 切换，和单面翻版同一套箭头；到最后一面再点 › 仍是重说当前这一面。
- 多面请求遇 `BATCH_STORAGE_QUOTA_EXCEEDED` 时，会先压缩本扩展的旧翻页缓存再登记一次。仍满则阻断请求，不发网络、不切 nostream、不清除站点数据。
- 覆盖安装后请完整刷新，确认运行版本 1.5.72。
