# 1.5.71 · TT 缺镜失败卡 + 可调扫描范围

- 缺壳扫描默认最近 10 楼，设置里可改为 20 楼或全部助手回复，并列出当前缺外壳的楼号。范围内缺镜的助手回复下面挂失败卡，仍不自动再发请求。
- TT 的 `#chat` 不能插兄弟节点，原先把外壳塞进横向 flex 的 `.mes`，失败卡会被挤没。现挂到正文栏（`.mes_text` 下面），闪退间隙也不再 1.8 秒删掉失败卡。
- 覆盖安装后请完整刷新，确认运行版本 1.5.71。

# 1.5.70 · 缺镜失败卡挂在助手回复下面

- 闪退或网络中断后没有授权记录，不再把失败卡一起挡住。独立 API 下，缺成品、缺外壳的助手回复正下方会挂失败卡，可点「重新生成兔子镜」，仍不自动再发请求。
- 最近 6 条、以及当前正在同步的可见楼层都会补；跟随镜、已有成品、用户删过的、正在生成、或生成方式关闭的楼层不补。
- 覆盖安装后请完整刷新，确认运行版本 1.5.70。

# 1.5.69 · 缺壳楼层可手动重试

- 独立 API 最近 6 条助手回复如果没有成品、也没有外壳（网络中断、闪退或页面退出后常见），会补一张失败卡，可点「重新生成兔子镜」。不自动再发请求。
- 已有成品、用户删过的、正在生成、跟随镜、或生成方式为关闭的楼层不补卡。加载占位不会把这张失败卡盖回「正在生成」。
- 覆盖安装后请完整刷新，确认运行版本 1.5.69。

# 1.5.68 · 1.5.57 以来

## 重说五版

- 副 API 成功重说按「每一面」最多保留 5 版，标题旁 `‹ n/n ›` 切换；到头或到尾点箭头就是重说，不必再进挨打猫。满五后必须先删当前这一版才能再重说，不会自动挤掉最旧。
- 五版跟着这层聊天、这楼、这张 swipe 走。重说生成新版后应变成 2/2、3/3，不会每次都 1/1。
- 重说失败只显示失败卡，不写入那 5 格；切回上一版或刷新后回到最近成功成品。失败卡上再点重说覆盖这一格失败。
- 维修不再另存版本。维修兔「恢复到初始」回到这一版刚生成时的整面 HTML，格子数不变；「返回修复前」和挨打猫「兔子镜历史」藏掉。

## 自动重 roll

- 本轮没有可用鲜兔镜时（空回、报错、掉格式，或 180 秒还没拿到完整内容）自动重发，默认最多 2 次额外请求，设置里可调。已出的 ready 镜面不会被失败卡或 loading 壳盖掉。
- 空闲 5 分钟或总等待 20 分钟仍是终点：不再自动重发，需手动重说。用尽后才提示「已自动重试 2/2」；第一次失败或因已有鲜面而跳过时不再误报。

## 收藏夹

- 本机收藏夹保存已净化的单面 HTML/文本，打开时走原净化和挂载管线，保留交互；不写入主楼 Prompt。从抽签「收藏室」拆出，设置首页单独一栏，镜面工具菜单也可「打开收藏夹」。列表按角色卡分组；旧收藏没有角色信息时归入「未分类」。
- 标题兔子按钮旁可点星标收藏，点第二次取消。收藏夹 / 回看是顶层对话框，能盖在设置上面；Esc 只关收藏夹。回看不当成聊天里的独立镜面。回看去掉标题上的翻页条、星标、兔/猫/骰子，并提供「复制 HTML」。
- 扩展魔杖菜单「兔子镜」带同款兔子图标，背景跟着酒馆主题走。

## 维修兔

- 菜单按游玩高频问题重排：点了没反应、展开后文字被裁、整面太窄。「展开后文字被裁」只测量当前已打开的抽屉／选项。「点了没有反应」修完后用人话回执。

## 其他

- TT 虚化：滚出屏幕只摘 DOM、保留缓存成品；滚入可见楼层时按 mesid 回挂，不发新请求。
