import { getSettings } from './settings.js?rmv=1.5.51-narrow1';
import { getCurrentChatKey } from './storage.js?rmv=1.5.51-narrow1';
import { getRabbitMirrorRecipe } from './blacklist.js?rmv=1.5.51-narrow1';
import { readFollowPartialResult, replaceFollowPartialResultFace } from './followPartialResults.js?rmv=1.5.51-narrow1';
...
import { parseMultifaceOutput, MULTIFACE_FAILURE_ATTR } from './multifaceProtocol.js?rmv=1.5.51-narrow1';
import { planRabbitMirrorPromptDetails, renderRabbitMirrorPromptPlan, prepareSelectedMemoryForPrompt, memoryRequestSettingsKey, assertMemoryRequestSettings } from './promptBuilder.js?rmv=1.5.51-narrow1';
import { hydrateExternalPoolMetadata, getSelectedExternalEntries } from './externalWorldBook/store.js?rmv=1.5.51-narrow1';
import { refreshRabbitMirrorToolsInScope, isolateRabbitMirrorInteractionIds } from './outputSanitizer.js?rmv=1.5.52-domcompat1';
import { authorizeRabbitMirrorIndependentServiceRequest, assertRabbitMirrorIndependentResponseText } from './independentSecurityGuard.js?rmv=1.5.51-narrow1';
...
export async function retryFollowFace(root, suppliedOwner, deps) {
    ...
    try {
        ...
        try {
            ...
            if(plan.appearanceReference.enabled){
                const appearance=await import('./appearanceReference.js?rmv=1.5.51-narrow1');assertCurrent();
    ...
    }catch(error){const message=error?.rabbitMirrorFollowRetry||/^RABBIT_MIRROR_(?:APPEARANCE|MEMORY)_/.test(String(error?.code||''))?error.message:'这一面重试失败或安全检查未通过；原有各面保留，未自动补发。';globalThis.toastr?.error?.(message);return {ok:false,error:message};}
